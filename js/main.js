   ...
   "use strict";
    ...
    $(document).ready(function(){ 
        ...
        customForm.each(function(){
            ...
            function submitForm(){  
                var password = thisSiteForm.find("#hero-form-password").val() || thisSiteForm.find("#footer-signup-password").val(),
                    email = thisSiteForm.find("#hero-form-email").val() || thisSiteForm.find("#footer-signup-email").val(),    
                    formhash =  thisSiteForm.find("input[name='formhash']").val();

                $.ajax({
                    type: "POST",
                    url: "/account/signin",
                    data: "task=login&email=" + email + "&password=" + password+ "&formhash=" + formhash,
                    dataType: 'json',
                    success : function(data){
                        if (data.status == "success"){
                            ...
                            formError();
                            submitMSG(false,data.email_hint+data.pwd_hint);
                        }
                    }
                });
            }
        ...
        // Small Screen Menu
        var smlScrnTogBtn = $('<span class="sml-scrn-tog-btn ion-grid">导航</span>'),
            closeMenu = $('<div class="close-menu"><span>一个帅气的导航</span></div>'),
